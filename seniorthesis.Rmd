...
cal_p_boot = function(data, X, B.max=1999, FUN=get_stats){
  ...
  while(b <= B.max){
    ...
    data.star[data.star[,'W2']==0, c(4,6)] = NA
    stat.star = FUN(data.star)[1]
    X.sq.S.star[b] = stat.star
  }
  p.value.boot = mean(X.sq.S.star >= stat)
  return(list(p_value=p.value.boot, X.sq=X.sq.S.star))
}
system.time(cal_p_boot(simulated_data, X=2))

parallel_p_boot = function(data, B.max, FUN=get_stat){
  stat = FUN(data=data)
  X.sq.S.star = numeric(length(B.max))
  n = dim(data)[1]
  end = dim(data)[2]
  ...
  X.sq.S.star[B.max] = stat.star
  p.value.boot = mean(X.sq.S.star >= stat)
  return(list(p_value=p.value.boot, X.sq=X.sq.S.star))
}
len = 1999
system.time(
  res = lapply(1:len, FUN=function(x){parallel_p_boot(data=simulated_data, B.max = x)})
)
```
```{r}
library(doSNOW)
...
B.max = 1999
X.sq.S.star = numeric(length(B.max))
system.time(X.sq.S.star = foreach(i = 1:B.max, .combine='rbind')%dopar%
  {
  stat = get_stats(data)
  n = dim(data)[1]
  end = dim(data)[2]
