...
```{r data.star: calculate j_count}
# print(data.star[1:10,])
j_count = matrix(0, nrow = 1, ncol = 2)
notj_count  = matrix(0, nrow=1, ncol=2)
for (indi in 1:nrow(data.star)){
  denom = sum(data.star[indi,c(1,2)])
  if (denom == 0){
    next
  }
  else if (data.star[indi,c(3,4)]=NA){
   denom = 1
   j_count[1,1] = sum(data.star[indi, c(3,4)])
  }
  j_count[1,1] = 
}
print(data.star[1:10,])
